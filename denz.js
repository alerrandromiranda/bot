...
const vcard2 = 'BEGIN:VCARD\n'
            ...
            + 'TEL;type=CELL;type=VOICE;waid=5511983040984:+55 11 98304 0984\n'
            + 'END:VCARD'
...
prefix = "."
name = "TONY BOT"
rdaftar = "OBRIGADO POR SE REGISTAR PARA SER UM USUÁRIO DO Screamobot 😁"
rmenu = "Oi amigo Screamobot 👋🏻 NÃO SE ESQUEÇA NAO PAGUE POR BOTS:)"
botinfo = "PARA BOTS CONVIDADOS, TODOS BOTS TERMUX SAO INDONÉSIO NENHUM BR CRIA SÓ EDITA:)"
limitt = 999999999
memberLimit = 2
ban = []
premium = ["554398434473@s.whatsapp.net","553288716525@s.whatsapp.net","5511983040984@s.whatsapp.net","558198904324@s.whatsapp.net"]
		...
		if(antifake.includes(anu.jid)) {
			if (anu.action == 'add'){
				...
				if(!num.split('@')[0].startsWith(55)) {
					denz.sendMessage(mdata.id, 'NUMEROS FAKES NÃO SÃO PERMITIDOS NESTE GRUPO, ENTRE COM UM OFICIAL +55', MessageType.text)
...
DIGITE ${prefix}daftar PARA COMEÇAR A USAR O BOT.`
				...
				teks = `*_「 NOVO ADMIN DETECTADO 」_*\n@${num.split('@')[0]} JA ESTA DE ADMIN, PROMOVIDO PELO ADM DO GRUPO!`
				let buff = await getBuffer(ppimg)
				denz.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
				...
				teks = `*_「 REBAIXAMENTO DE ADMIN DETECTADO」_*\n@${num.split('@')[0]} REBAIXADO PARA MEMBRO COMUM, REBAIXADO PELO ADMIN DO GRUPO`
				let buff = await getBuffer(ppimg)
				denz.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
				...
				teks = `ADEUS @${num.split('@')[0]}👋\nSE SAIU POR ALGUM INCOVENIENTE, DESCULPE, MAS SE FOR POR CAUSA DE PROBLEMAS, NÃO VOLTE`
				let buff = await getBuffer(ppimg)
				denz.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	denz.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '*ESPERE UM POUCO... SE DER ERRO TENTE NOVAMENTE*',
				success: '*SUCESSO*',
				error: {
					stick: '*ERRO, FALHA AO CONVERTER A IMAGEM EM ADESIVO.*',
					Iv: '*ESTE LINK ESTA INVÁLIDO!*'
				},
				only: {
					group: '*DESCULPE, ESTE COMANDO SÓ PODE SER USADO EM GRUPOS!*',
					benned: '*DESCULPE, SEU NÚMERO ESTÁ PROIBIDO DE USAR O BOT, POR FAVOR, ENTRE EM CONTATO COM O PROPRIETÁRIO DO BOT PARA LIBERAR O SEU ACESSO.*',
					ownerG: '*DESCULPE, ESTE ESTE COMANDO SO PODE SER USADO POR ADMINS EM GRUPOS !*',
					ownerB: '*DESCULPE, ESTE COMANDO SÓ PODE SER USADO PELO TONY, OTÁVIO, BIEL, NALDO.* ',
					...
					admin: '*DESCULPE, ESTE COMANDO PODE SER USADO SÓ PELOS ADM DO GRUPO, SEU MEMBRO COMUM 🤣',
					Badmin: '*DESCULPE, ESTE COMANDO SÓ PODE SER USADO QUANDO O BOT VIRA ADM DO GRUPO!*',
        ...
        if (adminNumber.includes(messagesLink)){
        	hasil = `        ────────────────
ʜᴇɪ *${pushname2}* POR FAVOR, DIGITE *${prefix}menu*
        ────────────────`
denz.sendMessage(from, hasil, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": "ketik #menu kak", 'jpegThumbnail': fs.readFileSync('./sticker/dnsnew.webp')}}}})
		...
		setTimeout( () => {
		reply(`*_「 𝑃𝑎𝑙𝑎𝑣𝑟𝑎 𝑜𝑓𝑒𝑛𝑠𝑖𝑣𝑎 𝐷𝑒𝑡𝑒𝑐𝑡𝑎𝑑𝑎」_*\n𝐷𝑒𝑠𝑐𝑢𝑙𝑝𝑒 *${pushname2}* Não permitimos palavrões ou Palavras ofensivas neste grupo. Você sera removido!*${groupMetadata.subject}*`)
		}, 0)
		}
       ...
       exec(`wget ${anu1} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
         ...
         case 'lirik':
                ...
                if (args.length < 1) return reply(`Mau request apa? Contoh: #request fitur anime`)
                    ...
                    denz.sendMessage('4398434473@s.whatsapp.net', options, text, {quoted: mek})
  ...
  break
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					if (mentioned.length > 1) {
						teks = 'COMANDO RECEBIDO! O ALVO SERÁ REMOVIDO POR NÃO SEGUIR AS REGRAS DO GRUPO :\n'
