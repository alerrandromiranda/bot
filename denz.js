...
const vcard2 = 'BEGIN:VCARD\n'
            ...
            + 'VERSION:3.0\n'
            + 'FN:Naldo-PE\n'
            + 'ORG:Co Owner Naldo 𝙱𝙾𝚃;\n'
            + 'TEL;type=CELL;type=VOICE;waid=558198904324:+55 81 9890 4324\n'
            + 'END:VCARD'
...
memberLimit = 3
ban = []
premium = ["554397434473@s.whatsapp.net","553288716525@s.whatsapp.net","5511957505042@s.whatsapp.net","558198904324@s.whatsapp.net"]
...
ᴅɪɢɪᴛᴇ ${prefix}daftar ᴘᴀʀᴀ ᴄᴏᴍᴇᴄᴀʀ ᴀ ᴜsᴀʀ  ʙᴏᴛ`
	...
	denz.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["554398434473@s.whatsapp.net","5511957505042@s.whatsapp.net","553288716525@s.whatsapp.net","558198904324@s.whatsapp.net"] // owner number ubah aja
		...
		setTimeout( () => {
		reply(`desculpe grupo Tony bot saia logo *${groupMetadata.subject}*`)
		}, 0)
		}
       ...
       exec(`wget ${anu1} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
         ...
         case 'lirik':
                ...
                if (args.length < 1) return reply(`Mau request apa? Contoh: #request fitur anime`)
                    ...
                    denz.sendMessage('554397434473@s.whatsapp.net', options, text, {quoted: mek})
                    reply('Problemas foram relatados ao proprietário do BOT, relatórios falsos não serão respondidos.')
                    break
				case 'memeindo':
				if (isBanned) return reply(mess.only.benned)    
...
break
                  ...
                  data = await fetchJson(`https://api.vhtear.com/randomwibu&apikey=${VthearApi}`)
				    ...
				    if (isLimit(sender)) return reply(limits.limitend(pushname2))
						...
						denz.sendMessage(from, buffer, image, {quoted: mek, caption: 'Hmmm lolizinha'})
					} catch (e) {
			    ...
			    case 'nsfwblowjob':
				    ...
				    if (isLimit(sender)) return reply(limitend(pushname2))
						...
						denz.sendMessage(from, buffer, image, {quoted: mek, caption: 'Loli :)'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(' *ERROR* ')
					}
			    ...
			    case 'nsfwneko':
				    ...
				    if (isLimit(sender)) return reply(limitend(pushname2))
						...
						buffer = await getBuffer(res.result)
						denz.sendMessage(from, buffer, image, {quoted: mek, caption: 'Ta ai'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(' *ERROR* ')
					}
...
quoted: mek, caption: `Hasil Dari *${daerah}*`
  ...
  break
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔═══✪ Marcando Os Boi Do Grupo *${pushname2}* ✪══`+ teks +'╚═══〘 *TONY BOT* 〙═══', members_id, true)
					break
			    case 'mentionall':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
				...
				case 'clearall':
					...
					reply(`\`\`\`Exclua todos os bate-papos com sucesso TONY BOT\`\`\``)
					break
                                case 'bcgc':
					denz.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(mess.only.ownerB)
...
denz.sendMessage(from, comd, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": "Menu do Tony #menu", 'jpegThumbnail': fs.readFileSync('./sticker/dnsnew.webp')}}}})
				  }
