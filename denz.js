...
const vcard1 = 'BEGIN:VCARD\n'
            + 'VERSION:3.0\n'
            + 'FN:Biel shark\n'
            + 'ORG:Co Owner Biel 𝙱𝙾𝚃;\n'
            + 'TEL;type=CELL;type=VOICE;waid=553288716525:+55 32 8861 6525\n'
            + 'END:VCARD'

const vcard2 = 'BEGIN:VCARD\n'
            + 'VERSION:3.0\n'
            + 'FN: Otávio\n'
            + 'ORG:Coo Owner Otávio 𝙱𝙾𝚃;\n'
...
rmenu = "Oi amigo do Tony:)"
botinfo = "FODA SE  :)"
limitt = 999999999
memberLimit = 3
ban = []
premium = ["554397434473@s.whatsapp.net","553288716525@s.whatsapp.net","5511957505042@s.whatsapp.net"]
...
ᴅɪɢɪᴛᴇ ${prefix}daftar ᴘᴀʀᴀ ᴄᴏᴍᴇᴄᴀʀ ᴀ ᴜsᴀʀ  ʙᴏᴛ`
	...
	denz.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["554398434473@s.whatsapp.net","5511957505042@s.whatsapp.net","553288716525@s.whatsapp.net"] // owner number ubah aja
        ...
        if (adminNumber.includes(messagesLink)){
        	hasil = `        ────────────────
 ᴇɪ *${pushname2}* ᴘᴏʀ ғᴀᴠᴏʀ ᴅɪɢɪᴛᴇ *${prefix}menu*
        ────────────────`
denz.sendMessage(from, hasil, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": "TONY #menu", 'jpegThumbnail': fs.readFileSync('./sticker/dnsnew.webp')}}}})
}
		if (bad.includes(messagesLink)) {
		if (!isGroup) return
		...
		setTimeout( () => {
		reply(`*_「 𝑃𝑎𝑙𝑎𝑣𝑟𝑎 𝑜𝑓𝑒𝑛𝑠𝑖𝑣𝑎 𝐷𝑒𝑡𝑒𝑐𝑡𝑎𝑑𝑎」_*\n𝐷𝑒𝑠𝑐𝑢𝑙𝑝𝑒 *${pushname2}* VOCÊ ESTÁ XINGANDO E SERÁ REMOVIDO*${groupMetadata.subject}*`)
		}, 0)
		}
       ...
       exec(`wget ${anu1} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
            ...
            break  
                 ...
                 case 'kalkulator':
				   ...
				   if (isLimit(sender)) return reply(limitend(pushname2))
				    ...
				    break 
				case 'dono':
                 denz.sendMessage(from, {displayname: "TONY", vcard: vcard}, MessageType.contact, { quoted: mek})
                 denz.sendMessage(from, '*_ᴅᴜᴠɪᴅᴀs sᴏʙʀᴇ ᴏ ʙᴏᴛ sᴀʟᴠᴇ ᴏ ɴᴜᴍᴇʀᴏ  :)_*',text, { quoted: mek} )
                 break
                 
                 case 'dono2':
                 denz.sendMessage(from, {displayname: "Tavin", vcard: vcard1}, MessageType.contact, { quoted: mek})
                 denz.sendMessage(from, '*_Duvidas sobre o bot pergunte >-<_*',text, { quoted: mek} )
                 break
                
            case 'dono3':
                 denz.sendMessage(from, {displayname: "Biel Janta", vcard: vcard1}, MessageType.contact, { quoted: mek})
                 denz.sendMessage(from, '*_Duvidas sobre o bot pergunte >-<_*',text, { quoted: mek} )
                 break     
         ...
         case 'lirik':
                ...
                if (args.length < 1) return reply(`Yang mau direport apaan? Contoh: #report fitur #hentai error`)
                    ...
                    denz.sendMessage('554398434473@s.whatsapp.net', options, text, {quoted: mek})
                    reply('SUA SOLICITAÇÃO ATINGIU O proprietário do BOT. Solicitações falsas/enganosas não serão respondidas..')
                    break
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
				reply(mess.wait)
					memein = await fetchJson(`https://br.pinterest.com/camilefischer/memes-em-portugues/`)
...
break
                  ...
                  data = await fetchJson(`https://api.vhtear.com/randomwibu&apikey=${VthearApi}`)
				    ...
				    if (isLimit(sender)) return reply(limits.limitend(pushname2))
						...
						denz.sendMessage(from, buffer, image, {quoted: mek, caption: '_LOLIZINHA MANO? DAQUI A POUCO O FBI TE FAZ UMA VISITINHA_'})
					} catch (e) {
				...
				case 'nsfwtrap':
				    ...
				    if (isLimit(sender)) return reply(limitend(pushname2))
						...
						buffer = await getBuffer(res.result)
						denz.sendMessage(from, buffer, image, {quoted: mek, caption: 'o último hein poha e para de pedir hentai safado (a)'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(' *ERROR* ')
					}
...
// only grup fitur anime
              case 'anime':
                ...
                if (isLimit(sender)) return reply(limitend(pushname2))
                if (!isAnime) return reply(' *Primeiro ative o Modo Anime* ')
					anu = await fetchJson(`https://tobz-api.herokuapp.com/api/randomanime?apikey=${TobzApi}`, {method: 'get'})
					reply(mess.wait)
					pok = await getBuffer(anu.result)
					denz.sendMessage(from, pok, image, { quoted: mek , caption: 'Ta ai'})
					await limitAdd(sender) 
					break  
				case 'naruto':
				if (isBanned) return reply(mess.only.benned)    
...
// akhir fitur anime
			...
			case 'hekerbucin':
				...
				case 'tiktokstalk':
					...
					console.log(`Error :`, color(e,'red'))
					reply('Possivelmente o nome de usuário é inválido ')
					}
					await limitAdd(sender) 
					break  
//creator
...
//akhir kreator
			...
			case 'katailham':
				...
				case 'addaudio':
                   if (!isOwner) return reply(mess.only.ownerB)
                   if (!isQuotedAudio) return reply('Marca o áudio Tony')
                   gsh = body.slice(10)
...
quoted: mek, caption: `Hasil Dari *${daerah}*`
  ...
  break
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔═══✪ Marcar Os Boi Do Grupo *${pushname2}* ✪══`+ teks +'╚═══〘 *DNS BOT* 〙═══', members_id, true)
					break
			    case 'mentionall':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
				...
				if (!isGroup) return reply(mess.only.group)
					...
					if ((args[0]) === 'on') {
						...
						fs.writeFileSync('./database/json/antilink.json', JSON.stringify(antilink))
						reply(`\`\`\`✓“Desativado com sucesso o recurso anti-link no grupo\`\`\` *${groupMetadata.subject}*`)
					} else {
...
case 'husbu':
			...
			break
				...
				if (budy.includes("://chat.whatsapp.com/")){
					if (!isGroup) return
					if (!isAntiLink) return
					if (isGroupAdmins) return reply(`*${pushname2}* NÃO POSSO REMOVER OS ADM`)
					denz.updatePresence(from, Presence.composing)
					if (messagesLink.includes("#izinadmin")) return reply("#izinadmin *_「 recebido 」_*")
